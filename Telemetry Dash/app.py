...
# # df = pd.read_excel('location_history.xls')
# # df[['latitude', 'longitude']] = df['Coordinates/Address'].str.split(',', expand=True)
...
# # df['datetime'] = pd.to_datetime(df['Date and Time'], format='%Y-%m-%d %H:%M:%S')
# # df["date"] = pd.to_datetime(df['Date and Time'], format='%Y-%m-%d').dt.date
# # df["date"] = pd.to_datetime(df['date'], format='%Y-%m-%d')
...
# # # def speedlValue():
# # #     speed_value  = df["Fuel (A/D Value /L)"].iloc[-1]
    
# #     # if speed_value < 75:
# #     #     speed_value  = df["Fuel (A/D Value /L)"].iloc[-1]
# #     # else:
# #     #     speed_value = "Over Speed"
        
# #     # return speed_value    

# # # def fuelValue():
# # #     fuel_value  = df["Fuel (A/D Value /L)"].iloc[-1]
    
# #     # if fuel_value > 10:
# #     #     fuel_value  = df["Fuel (A/D Value /L)"].iloc[-1]
# #     # else:
# #     #     fuel_value = "Refill Tank"
        
# #     # return fuel_value    


# # # def batteryValue():
# # #     battery_value = df["Voltage/Battery (V/%)"].iloc[-1]
    
# #     # if battery_value > 12:
# #     #     battery_value = df["Voltage/Battery (V/%)"].iloc[-1]
# #     # else:
# #     #     battery_value = "Low Voltage"
        
# #     # return battery_value

# # # def engineTemp():
# # #     temp_value = df["Temperature (℃)"].iloc[-1]
...
# #                                 dcc.DatePickerSingle(
# #                                     id='my-date-picker-start',
# #                                     min_date_allowed=date(2023, 6, 13),
# #                                     max_date_allowed = date.today(),
# #                                     # max_date_allowed=date(2018, 1, 24),
# #                                     initial_visible_month=date(2023, 6, 14),
# #                                     date=date(2023, 6, 14)
# #                                 ),
# #                                 dcc.DatePickerSingle(
# #                                     id='my-date-picker-end',
...
# #                                     max_date_allowed = date.today(),
# #                                     # max_date_allowed=date(2018, 1, 24),
# #                                     initial_visible_month=date(2023, 6, 17),
# #                                     date=date(2023, 6, 17),
# #                                 ),
# #                             ])
# #                         ]),
...
# #     Input('interval-component', 'n_intervals'),
# # )
# # def update_graphAll(start_dateAll, n):
# #     dff = df.copy()
# #     dff1 = dff[(dff.date >= start_dateAll)]
# #     # dff.loc[((dff.date>=start_date) & (dff.date<=end_date)), 'colors'] = 'black'
# #     figAll = px.scatter_mapbox(dff1,
# #                         lon = dff1['longitude'],
# #                         lat = dff1['latitude'],
...
# #     Input('interval-component', 'n_intervals'),
# # )
# # def update_graph1(start_date1, end_date1, n):
# #     dff = df.copy()
# #     dff2 = dff[(dff.date >= start_date1) & (dff.date <= end_date1)]
# #     # dff.loc[((dff.date>=start_date) & (dff.date<=end_date)), 'colors'] = 'black'

# #     fig1 = px.line(dff2, y='Speed (Km/h)', x='datetime', title ='SPEED WITH TIME')
# #     fig1.update_traces( line_color='#FF5E5E')
# #     fig1.update_layout(margin = {'r':0, 't':0, 'l':0, 'b':0}, height=300,width=3500,)


# #     fig2 = px.line(dff2, y='Fuel (A/D Value /L)', x='Mileage (Km)', title ='FUEL CONSUMPTION WITH DISTANCE')
# #     fig2.update_traces( line_color='#FF5E5E')
# #     fig2.update_layout(margin = {'r':0, 't':0, 'l':0, 'b':0}, height=300,width=3500,)

# #     fig4 = px.line(dff2, y='Fuel (A/D Value /L)', x='datetime', title ='FUEL CONSUMPTION TEMP WITH TIME')
# #     fig4.update_traces( line_color='#FF5E5E')
# #     fig4.update_layout(margin = {'r':0, 't':0, 'l':0, 'b':0}, height=300, width=3500,)
...
# #     mileage = df["Mileage (Km)"].iloc[-1].round(),
# #     def engineTemp():
# #         temp_value = df["Temperature (℃)"].iloc[-1]
# #         return temp_value
